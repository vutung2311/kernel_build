...
function create_modules_staging() {
  ...
  cp ${src_dir}/modules.alias ${dest_dir}/modules.alias
  cp ${src_dir}/modules.order ${dest_dir}/modules.order
  cp ${src_dir}/modules.builtin ${dest_dir}/modules.builtin
  cp ${src_dir}/modules.dep ${dest_dir}/modules.dep
  cp ${src_dir}/modules.softdep ${dest_dir}/modules.softdep
  ...
  cp ${dest_dir}/modules.order ${dest_dir}/modules.load.recovery
}
...
if [ -n "${BUILD_BOOT_IMG}" -o -n "${BUILD_VENDOR_BOOT_IMG}" ] ; then
  ...
  if [ -z "${DTB_FILE_LIST}" ]; then
    ...
    if [ -n "${DTB_PREBUILT_BINARY}" ]; then
      MKBOOTIMG_ARGS+=("--dtb" "${DTB_PREBUILT_BINARY}")
    else
